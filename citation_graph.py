...
class CitationGraph:
    def __init__(self, doi_lst, max_age=30, min_gap=0.1):
        assert max_age > 7
        self.max_age = max_age
        self.curr_refs = dict()  # entry: 'id':[Reference, local_id_set]
        self.doi_list = []
        for doi in doi_lst:
    ...
    def print_refs(self):
        tab_head = ["#", "local cites", "total cites", " " * 50 + "title", "cover date",
                    " " * 30 + "source title", ]
        col_gap = []
        fmt = ""
        for hd in tab_head:
        ...
        dat.sort(key=lambda x: (len(x[1]) if x[1] else 0, int(x[0].citedbycount) if x[0].citedbycount else 0,),
                 reverse=True)

        for i, itm in enumerate(dat):
            print(fmt % (str(i + 1)[:col_gap[0]],
                         str(len(itm[1]))[:col_gap[1]],
                         str(itm[0].citedbycount)[:col_gap[2]],
                         str(itm[0].title)[:col_gap[3]],
                         str(itm[0].publicationyear)[:col_gap[4]],
